use std::fs;
use std::os::unix::fs::PermissionsExt;
use std::path::Path;

use crate::config::{AccountsFile, Config};
use crate::error::VivariumError;

const DEFAULT_CONFIG: &str = r#"[defaults]
# mail_root = "~/.vivarium"
# check_interval_secs = 300
# Semantic search/indexing is opt-in. Set these to your own embedding service.
# embedding_provider = "ollama"
# embedding_model = "your-embedding-model"
# embedding_endpoint = "http://your-embedding-host/api/embed"
"#;

const DEFAULT_ACCOUNTS: &str = r#"# Each [[accounts]] entry defines an email account.
# Passwords are stored here — keep this file chmod 600.

# Proton Bridge (recommended for local archive):
# vivi-mail defaults to 127.0.0.1:1143 (IMAP SSL) and
# 127.0.0.1:1025 (SMTP STARTTLS) for Protonmail.
# Self-signed certificates are accepted by default for Bridge.
# Use the mailbox password generated by Bridge, not your main Proton password.
# [[accounts]]
# name = "proton"
# email = "you@proton.me"
# username = "you@proton.me"
# auth = "password"
# password = "your-bridge-app-password"
# provider = "protonmail"
# imap_security = "ssl"
# smtp_security = "starttls"

# Experimental direct Proton API account for isolated containers:
# [[accounts]]
# name = "agent-proton"
# email = "agent@proton.me"
# username = "agent@proton.me"
# auth = "password"
# password_cmd = "printenv PROTON_PASSWORD"
# provider = "proton-api"
# storage_mode = "semantic"

# Gmail:
# [[accounts]]
# name = "gmail"
# email = "you@gmail.com"
# username = "you@gmail.com"
# auth = "xoauth2"
# provider = "gmail"
# oauth_client_id = "your-google-oauth-client-id.apps.googleusercontent.com"
# oauth_client_secret = "your-google-oauth-client-secret"
# token_cmd = "vivi-mail token gmail"
# imap_host = "imap.gmail.com"
# imap_security = "ssl"

# Custom IMAP:
# [[accounts]]
# name = "custom"
# email = "you@example.com"
# username = "you@example.com"
# auth = "password"
# password = "your-password"
# provider = "standard"
# imap_host = "imap.example.com"
# imap_port = 993
# imap_security = "ssl"
"#;

const DEFAULT_AGENT_PROMPT: &str = r"You are processing instructions delivered through the vivi-mail trusted agent mailbox.
Use your judgment about what action, if any, is appropriate. After processing, send a reply in this same thread from the receiving vivi-mail account summarizing what action you took, or explaining that no action was taken.
Use vivi-mail's draft/send surfaces for the reply and send from the receiving account. Do not send from another account unless the instruction explicitly asks you to.

To customize per account, create:
  ~/.vivarium/agent/prompts/<account-name>.md
";

/// Initialize the vivi-mail directory structure, config, and accounts file.
///
/// # Errors
/// Returns an error if filesystem operations (create dir, write file, set permissions) fail.
///
/// # Panics
/// Panics if the default config path has no parent directory (indicates a
/// malformed system environment).
pub fn run_init() -> Result<(), VivariumError> {
    let config_path = Config::default_path();
    let accounts_path = AccountsFile::default_path();
    let dir = config_path.parent().expect("config path has parent");

    if !dir.exists() {
        fs::create_dir_all(dir)?;
        println!("created {}", dir.display());
    }

    write_if_missing(&config_path, DEFAULT_CONFIG)?;
    write_if_missing(&accounts_path, DEFAULT_ACCOUNTS)?;
    write_if_missing(&dir.join("agent").join("prompt.md"), DEFAULT_AGENT_PROMPT)?;

    // Ensure accounts.toml is 600
    fs::set_permissions(&accounts_path, fs::Permissions::from_mode(0o600))?;

    println!();
    println!("vivi-mail is ready. Next steps:");
    println!("  1. Edit {} and add an account", accounts_path.display());
    println!("  2. Run: vivi-mail sync");

    Ok(())
}

fn write_if_missing(path: &Path, content: &str) -> Result<(), VivariumError> {
    if path.exists() {
        println!("exists  {}", path.display());
    } else {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)?;
        }
        fs::write(path, content)?;
        println!("created {}", path.display());
    }
    Ok(())
}

#[cfg(test)]
#[path = "init_test.rs"]
mod tests;
